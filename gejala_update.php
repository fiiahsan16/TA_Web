    ...
    if (empty($error)) {
        if ($action == 'add') {
            if (mysqli_num_rows(mysqli_query($con, "select * from gejala where kode_gejala='" . $kode_gejala . "'")) > 0) {
                ...
                $q = "insert into gejala(kode_gejala,nama_gejala,nilai_ds) values ('" . $kode_gejala . "','" . $nama_gejala . "','" . $nilai_ds . "')";
                mysqli_query($con, $q);
        ...
        if ($action == 'edit') {
            ...
            if (mysqli_num_rows(mysqli_query($con, "select * from gejala where kode_gejala='" . $kode_gejala . "' and kode_gejala<>'" . $kode_gejala_tmp . "'")) > 0) {
                ...
                $q = "update gejala set kode_gejala='" . $kode_gejala . "',nama_gejala='" . $nama_gejala . "',nilai_ds='". $nilai_ds ."' where id_gejala='" . $id . "'";
                mysqli_query($con, $q);
                exit("<script>location.href='" . $link_data . "';</script>");
            }
        }
...
<div class="box box-success">
    ...
    <form class="form-horizontal" action="<?php echo $link_update; ?>" method="post">
        ...
        <div class="box-body">
            ...
            <div class="form-group">
                <label for="nilai_ds" class="col-sm-2 control-label">Nilai Theta</label>
                <div class="col-sm-4">
                    <input name="nilai_ds" id="nilai_ds" class="form-control" required type="text" value="<?php echo $nilai_ds; ?>">
            </div>
            </div>
            </div>
        </div>
